...
projects_data = {
    'carstrims-car-dealer-platform': {
        'title': 'CARSTRIMS — Multi-Tenant Car Dealer SaaS Platform',
        'tagline': 'A full-scale SaaS platform connecting car dealers, partners, staff, and buyers across Nigeria.',
        'client_type': 'SaaS / Automotive / Nigerian Market',
        'category': 'web-apps',
        'thumbnail': 'projects/carstrims_thumbnail.jpg',
        'overview': (
            'CARSTRIMS is a production-grade, multi-tenant car dealer management SaaS platform built for the Nigerian '
            'automotive market. The platform brings together five distinct user roles — Super Admin, Dealer Admin, '
            'Dealer Staff, Partner/Asset Owner, and Public Buyer — into a single cohesive system with role-based '
            'dashboards and granular permissions.\n\n'
            'Dealer Admins manage full inventory (up to 10 photos + video per car), track sales with profit/loss '
            'reporting, manage expenses, onboard staff with permission control, handle partner relationships, '
            'respond to buyer requests, confirm appointments, log vehicle movements, export financial PDF reports, '
            'and generate QR codes per dealership.\n\n'
            'Public users browse a live car feed with advanced filtering (brand, price, condition, transmission, fuel, '
            'state, year, color), like and save cars, place special requests, book appointments, and communicate via '
            'a full messaging system. Push notifications with sound and a service worker keep all users updated in '
            'real time. The platform is deployed on Vercel (frontend) and Render/Railway (backend API), with '
            'Cloudinary handling media storage and MongoDB Atlas as the primary database.'
        ),
        'technologies': [
            'Next.js 14', 'TypeScript', 'Zustand', 'Axios',
            'FastAPI (Python)', 'Motor (async MongoDB)', 'MongoDB Atlas',
            'Cloudinary', 'JWT Auth', 'Web Push API', 'Service Worker',
            'Tailwind CSS', 'Vercel', 'Render'
        ],
        'expertise': [
            'Architecting a production multi-tenant SaaS with 5 distinct role-based dashboards and permission systems.',
            'Building a FastAPI async backend with Motor for high-performance MongoDB operations.',
            'Implementing Web Push API with a service worker for real-time browser notifications with sound.',
            'Designing a full dealership management suite: inventory, sales, expenses, staff, partner workflows.',
            'Building a public car discovery feed with advanced multi-filter search and QR code integration.',
            'Handling Cloudinary media pipelines for up to 10 images + video per car listing.',
            'Deploying a distributed architecture across Vercel and Render with JWT token-based auth.',
            'Implementing real-time messaging with polling and a full conversation management system.',
        ],
        'features': [
        ...
        'problem_solution': {
            'problem': (
                'Nigerian car dealerships lacked a unified digital platform for inventory, sales tracking, '
                'partner management, and customer engagement. Existing tools were not tailored for the '
                'multi-dealer, multi-role complexity of the local automotive market.'
            ),
            'solution': (
                'CARSTRIMS was built from the ground up as a full SaaS solution — a single platform that handles '
                'the entire lifecycle of a car dealership: from inventory listing and partner asset tracking, '
                'to buyer discovery, appointment booking, and financial reporting.'
            ),
        },
        'downloads': [],
    },

    'bloodlink-blood-donor-management': {
        'title': 'BloodLink — Blood Donor Management System',
        'tagline': 'A full-stack platform connecting blood donors, beneficiaries, and healthcare administrators.',
        'client_type': 'Healthcare / NGOs / Blood Banks',
        'category': 'web-apps',
        'thumbnail': 'projects/blood_donor_thumbnail.jpg',
        'overview': (
            'BloodLink is a production-ready full-stack web application that digitises the entire blood donation '
            'process — from donor registration and verification to blood request fulfilment and real-time '
            'email notifications.\n\n'
            'The system connects three user groups: blood donors who register their blood group and availability; '
            'beneficiaries (patients or their representatives) who submit and track blood requests end-to-end; '
            'and administrators who verify donors, approve or reject requests, match donors to patients, and '
            'manage blood inventory across Nigerian states.\n\n'
            'Every key action triggers an automated email notification via Resend. Donors earn a printable '
            'certificate of appreciation upon verification. The platform eliminates the manual, phone-based '
            'approach used by hospitals and blood banks, replacing it with a transparent, trackable digital '
            'workflow where every stakeholder is notified in real time.'
        ),
        'technologies': [
            'Next.js 14 (App Router)', 'TypeScript', 'Tailwind CSS',
            'Next.js API Routes', 'MongoDB Atlas', 'Mongoose',
            'NextAuth.js (JWT)', 'Resend (Email)', 'Vercel'
        ],
        'expertise': [
            'Designing a three-role system (Admin, Donor, Beneficiary) with NextAuth.js JWT session management.',
            'Building a full blood request workflow: Pending → Approved → Matched → Fulfilled with admin controls.',
            'Implementing automated transactional emails at every workflow step using Resend.',
            'Designing a public donor search with blood group compatibility logic (e.g. O- as universal donor).',
            'Building a blood inventory management system across Nigerian states with colour-coded stock levels.',
            'Creating a printable donor certificate with print-optimised CSS layout.',
            'Ensuring full mobile responsiveness with a bottom navigation bar and sidebar drawer on mobile.',
            'Protecting all routes with role-based middleware redirects.',
        ],
        'features': [
            'Public donor search by blood group and Nigerian state with compatibility matching',
            'Donor registration with admin verification before appearing in search results',
            'Beneficiary blood request submission with urgency levels (Low/Medium/High/Critical)',
            'Full admin request workflow: Approve → Match Donor → Mark Fulfilled',
            'Blood inventory management across all 8 blood groups and Nigerian states',
            'Automated email notifications at every step for donors and beneficiaries',
            'Donor availability toggle (Available / Unavailable / On Cooldown)',
            'Printable certificate of appreciation for verified donors',
            'Role-based access control with protected route middleware',
            'Fully responsive mobile layout with bottom navigation',
            'Visual progress tracker on beneficiary dashboard',
            'Urgency and status colour-coded badges across all dashboards',
        ],
        'screenshots': [
            'blood0.jpg', 'blood1.jpg', 'blood2.jpg', 'blood3.jpg', 'blood4.jpg',
            'blood5.jpg', 'blood6.jpg', 'blood7.jpg', 'blood8.jpg', 'blood9.jpg',
            'blood10.jpg', 'blood11.jpg', 'blood12.jpg', 'blood13.jpeg',
        ...
        'problem_solution': {
            'problem': (
                'Blood shortages and delayed donor discovery during emergencies. Hospitals rely on manual, '
                'phone-based coordination with no visibility into donor availability or request status.'
            ),
            'solution': (
                'BloodLink provides a transparent, digital workflow where donors register and set availability, '
                'beneficiaries submit and track requests, and administrators coordinate the full process — all '
                'with automated email notifications at every step.'
            ),
        },
        'downloads': [],
    },
...
def portfolio(request):
    # Reorder so CARSTRIMS and BloodLink are always first
    priority_keys = ['carstrims-car-dealer-platform', 'bloodlink-blood-donor-management']
    ordered = {}
    for k in priority_keys:
        if k in projects_data:
            ordered[k] = projects_data[k]
    for k, v in projects_data.items():
        if k not in priority_keys:
            ordered[k] = v
    return render(request, 'core/portfolio.html', {'projects': ordered})
...
def custom_500(request): 
    return render(request, 'core/500.html', status=500), 'film-house9.jpg', 'film-house10.jpg', 'film-house11.jpg', 'film-house12.jpg',
        'film-house13.jpg'
    ],
    'live_demo_link': 'https://film-house-eight.vercel.app/',
    'github_link': 'https://github.com/Mr-Usty/film-house',
...
def custom_500(request): 
    return render(request, 'core/500.html', status=500)nd lack of vendor integration. '
            'Existing city-wide food apps are not optimized for campus operations.'
        ),
        'solution': (
            'UNILAGEats was developed as a campus-first food delivery solution, providing a controlled, '
...
def custom_500(request): 
    return render(request, 'core/500.html', status=500) backend systems or third-party form handling services, providing a seamless registration process for attendees.",
        'technologies': ['HTML', 'CSS', 'JavaScript', 'MS Office Suite (for data handling)', 'CorelDRAW (for form design elements)'],
        'thumbnail': 'projects/3-days-training_thumbnail.jpg',
        'expertise': [
            'Designing user-friendly and intuitive forms for efficient and accurate data collection.',
